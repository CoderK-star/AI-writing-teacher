...
import { listWorldSettings, createWorldSetting } from '@/lib/db/world-repository';
...
export async function GET(_req: Request, { params }: Params) {
  try {
    ...
    return Response.json(await listWorldSettings(projectId));
  } catch {
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const setting = await createWorldSetting({ id: randomUUID(), projectId, ...body });
