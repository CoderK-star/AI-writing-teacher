...
- プロット改善案を提示した後、「次に書くべきシーン」の冒頭叩き台（200字以内）を生成する。
  叩き台はあくまで書き出しのヒントとして位置づけ、ユーザーが自由に改変できる余白を残すこと。
  「続きはご自身でどうぞ！」などの一言を添えて、依存を防ぐ。`;
...
export class PlotAgent implements Agent {
  ...
  async execute(context: AgentContext): Promise<AgentResult> {
    ...
    const systemPrompt = [
      ...
      '',
      '出力形式: 1. 現状のプロット要約 / 2. 強みと課題 / 3. 具体的な改善案 / 4. 次に書くべきシーン / 5. シーン冒頭の叩き台（200字以内・書き出しのヒント）',
    ].join('\n');
